...
public class ModeBox extends Element {
	...
	public ModeBox(ModuleButton iparent, Setting iset) {
		parent = iparent;
		set = iset;
		super.setup();
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		String mode = clickgui.settingsManager.getSettingByName(parent.mod, set.getName()).getString();
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		if (mouseButton == 0) {
			if (isButtonHovered(mouseX, mouseY)) {
	            ...
	            String currentVal = clickgui.settingsManager.getSettingByName(parent.mod, set.getName()).getString();
	            int currentIndex = -1;
	            ...
	            if (currentIndex != -1) {
	                ...
	                clickgui.settingsManager.getSettingByName(parent.mod, set.getName()).setString(options.get(nextIndex).toLowerCase());
	            } else {
	                clickgui.settingsManager.getSettingByName(parent.mod, set.getName()).setString(options.get(0).toLowerCase());
	            }
	            return true;
	        }
		}
